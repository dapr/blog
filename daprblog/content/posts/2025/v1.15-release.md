---
date: "2025-02-10T07:00:00-07:00"
...
The workflow engine has undergone a major overwrite to achieve substantial performance and scalability improvements, as well as reliability and stability traits that are required to run mission critical apps at massive scale.

### Scheduler Service (stable)
The [Scheduler service](https://v1-15.docs.dapr.io/concepts/dapr-services/scheduler/) is the control plane service used to schedule jobs, running in self-hosted mode or on Kubernetes. It is installed when using the Dapr CLI and is the service responsible for managing scheduled jobs. The Schedulur service is now stable
...
## CLI and Env Vars
- *** TBD*** Ability to injected secrets as env variables into the Dapr sidecar to support values such as Auth tokens 
...
`http-read-buffer-size` and `http-max-request-size` darpd arguments are deprecated and replaced with `read-buffer-size` and `max-body-size`. See https://github.com/dapr/docs/pull/4401 and [annotations reference](https://v1-15.docs.dapr.io/reference/arguments-annotations-overview/)