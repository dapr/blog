---
date: "2024-03-01T07:00:00-07:00"
...
[Component "Hot Reloading"](https://v1-13.docs.dapr.io/operations/components/component-updates/#hot-reloading-preview-feature) is a new [preview feature](https://v1-13.docs.dapr.io/operations/support/support-preview-features/) that, when enabled, allows Component updates to be picked up automatically without the need for restarting the Dapr process. Component spec is reconciled and takes effect when running in both Kubernetes and Self-Hosted modes.

### Go SDK support for workflow

You can now use Go to write fault-tolerant, workflow based apps with durable execution. See the [Go SDK for more details](https://github.com/dapr/go-sdk/tree/main/examples/workflow) and try the [Quickstart](https://github.com/dapr/quickstarts/tree/release-1.13/workflows/go/sdk).

### Javascript SDK support for workflow

You can now use Javascript/Typescript to write fault-tolerant, workflow based apps with durable execution. See the [Javascript SDK for more details](https://github.com/dapr/js-sdk/tree/main/examples/workflow/authoring) and try the [Quickstart](https://github.com/dapr/quickstarts/tree/release-1.13/workflows/javascript/sdk).

### Rust SDK support for actors (alpha SDK)

You can now use Rust to run Dapr Actors, a programming model for highly scalable stateful applications. See the [Rust SDK for more details](https://github.com/dapr/rust-sdk/tree/master/examples/actors)

### Low metrics cardinality for HTTP

Dapr now includes an optional setting to enable low cardinality for HTTP metrics, useful for saving bandwidth and costs when dealing with high cardinality metrics. See additional info [here](https://v1-13.docs.dapr.io/operations/observability/metrics/metrics-overview/#high-cardinality-metrics).

### Graceful shutdown

Dapr now accepts a `dapr.io/block-shutdown-duration` annotation or `--dapr-block-shutdown-duration` flag which when set, will delay the full shutdown procedure for this duration or until the app reports as unhealthy- whichever is sooner. During this period all Subscriptions and input Bindings will be closed. This is useful for applications that need to use the Dapr APIs as part of their own shutdown procedure. See additional info [here](https://v1-13.docs.dapr.io/reference/arguments-annotations-overview/).

### Standardized error codes

The Dapr PubSub and State APIs now have the appropriate and standardized error codes returned to applications including enriched error details based on gRPC's richer error model. The remaining APIs are a work in progress, and community contributions towards this effort are greatly appreciated and very impactful. See additional info on the error codes [here](https://docs.dapr.io/reference/api/error_codes/). Check in the sdk docs for per sdk error code parsing and handling. See an [example of the Go SDK error parsing and handling here](https://v1-13.docs.dapr.io/developing-applications/sdks/go/go-client/#error-handling). 

### Actor reminders performance improvement

You can now opt-in to make actor reminders data use protobuf serialization instead of JSON. This increases throughput and reduces latency, potentially saving costs when using a managed cloud database as an actor state store. This serialization method will become default in v1.14.

***Note that once you turn this feature on, you should not downgrade Dapr to an earlier version as your reminders data will be unreadable.***

To enable protobuf serialization for actor reminders, set the following Helm argument on Kubernetes: ```dapr_placement.maxActorApiLevel=-1```. On self-hosted mode, run daprd with the flag ```--max-api-level=-1```.

### Components

#### Local name resolver based on SQLite 

You can now use a SQLite based name resolver for [service invocation in self-hosted](https://docs.dapr.io/developing-applications/building-blocks/service-invocation/service-invocation-overview/) mode. This is good for dealing with corporate firewalls and VPNs that filter mDNS. See additional info [here](https://v1-13.docs.dapr.io/reference/components-reference/supported-name-resolution/nr-sqlite/).

#### Postgres state store v2

Postgres has a new v2 implementation which contains improvements to performance and reliability. New applications are encouraged to use v2. The v1 implementation remains and is not deprecated. See additional info [here](https://v1-13.docs.dapr.io/reference/components-reference/supported-state-stores/setup-postgresql-v2/).

#### Azure blob storage state store v2

Azure Blob Storage has a new v2 implementation which is recommended. See additional info [here](https://v1-13.docs.dapr.io/reference/components-reference/supported-state-stores/setup-azure-blobstorage/#versioning).
...
- **ADDED** Version Skew Integration Tests [7301](https://github.com/dapr/dapr/pull/7301)
- **ADDED** PubSub API: standardize err msgs [7322](https://github.com/dapr/dapr/pull/7322) [6068](https://github.com/dapr/dapr/pull/6068)
...
- **ADDED** Calculate vnodes on daprd instead of the placement service [7382](https://github.com/dapr/dapr/pull/7382)
- **FIXED** Workflow Sub-Orchestration creation not working [7384](https://github.com/dapr/dapr/issues/7384)
- **ADDED** Update placement API Level gRPC context key to `dapr-placement-api-level` [7400](https://github.com/dapr/dapr/pull/7400)
- **FIXED** conflict with `content-length` metadata in pubsub subscription. [7413](https://github.com/dapr/dapr/issues/7413)
- **FIXED** Fix vnode calculation in placement clients. [7415](https://github.com/dapr/dapr/pull/7415)
...
- **FIXED** Placement service lock optimisations and race condition fix [7428](https://github.com/dapr/dapr/pull/7428)
- **FIXED** Fixes minimum observed cluster api level. Fixes api level check for vnodes. Adds more unit and integration tests for placement service.  [7462](https://github.com/dapr/dapr/pull/7462)
- **ADDED** Block Graceful Shutdown: stop input bindings and subscriptions [7474](https://github.com/dapr/dapr/pull/7474)
- **FIXED** Workflow grpc start API: Generate instanceID if not present [7503](https://github.com/dapr/dapr/pull/7503)
- **FIXED** Add content-length to http channel [7537](https://github.com/dapr/dapr/pull/7537)
...
- State Store: PostgreSQL v2 [2956](https://github.com/dapr/components-contrib/issues/2956) [3250](https://github.com/dapr/components-contrib/pull/3250)  
  This new version of the PostgreSQL state store can offer up to 10% improved performance in some scenarios. This version supports all stable capabilities, including Actor State Store; support for the Alpha Query API.  
...
- **ADDED** Add holopin.yml config for GHC2023 event [1147](https://github.com/dapr/dotnet-sdk/pull/1147)
- **ADDED** Proposal: Strongly-typed non-remoted Actor clients [1158](https://github.com/dapr/dotnet-sdk/issues/1158)
- **FIXED** Correct spelling of "identified" [1159](https://github.com/dapr/dotnet-sdk/pull/1159)
...
- **UPDATED** Updates Dapr to 1.12 in GitHub actions itest [1185](https://github.com/dapr/dotnet-sdk/pull/1185)
- **ADDED** Support .NET 8 [1187](https://github.com/dapr/dotnet-sdk/issues/1187)
- **FIXED** [Workflow] Examples do not work due to alpha -> beta changes [1189](https://github.com/dapr/dotnet-sdk/issues/1189)
- **FIXED** Modify broken links in README [1190](https://github.com/dapr/dotnet-sdk/pull/1190)
...
- **FIXED** Fix example dotnet-actors-howto.md [1218](https://github.com/dapr/dotnet-sdk/pull/1218)
- **FIXED** Update _index.md by fixing broken link [1221](https://github.com/dapr/dotnet-sdk/pull/1221)
...
- **UPDATED** Move to v4 of the upload/download artifact actions. [1240](https://github.com/dapr/dotnet-sdk/pull/1240)
...
- **FIXED** flaky integration test for Actor Reminder [553](https://github.com/dapr/js-sdk/pull/553)
- **FIXED** e2e test http actortest [554](https://github.com/dapr/js-sdk/pull/554)
...
-  **FIXED** e2e commom/server test [555](https://github.com/dapr/js-sdk/pull/555)
...
- As part of the Richer Error Codes model change ([7257](https://github.com/dapr/dapr/pull/7257), [7322](https://github.com/dapr/dapr/pull/7322)), a subset of the State and PubSub API errors has been updated to use an appropriate status code (ex. `INVALID_ARGUMENT`, `FAILED_PRECONDITION`) instead of the generic `UNKNOWN`
